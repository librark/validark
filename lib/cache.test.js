...
describe('cache', () => {
  ...
  it('caches a callable results for a certain time', async () => {
    ...
    expect(await cached.add({ first: 3, second: 4, third: 5 })).toEqual({ data: 12 })
    expect(await cached.add({ first: 3, second: 4, third: 5 })).toEqual({ data: 12 })
    expect(await cached.add({ first: 3, second: 4, third: 5 })).toEqual({ data: 12 })
    expect(calls.length).toEqual(1)
  ...
  it('has a size equal to the last recently used entries', async () => {
    ...
    expect(await cached.add({ first: 1, second: 2, third: 3 })).toEqual({ data: 6 })
    expect(await cached.add({ first: 1, second: 2, third: 3 })).toEqual({ data: 6 })
    expect(await cached.add({ first: 4, second: 5, third: 6 })).toEqual({ data: 15 })
    expect(await cached.add({ first: 7, second: 8, third: 9 })).toEqual({ data: 24 })
    expect(await cached.add({ first: 1, second: 2, third: 3 })).toEqual({ data: 6 })
    expect(await cached.add({ first: 1, second: 2, third: 3 })).toEqual({ data: 6 })
