...
it('does not parse non-graphql strings without graphql format', () => {
  const input = 'not a graphql statement'
  const result = structure(input, 'text')

  assert.equal(result, input)
})

it('does not parse non-operation strings when no format is provided', () => {
  const input = 'just plain content'
  const result = structure(input)

  assert.equal(result, input)
})

it('parses root selections with explicit graphql format', () => {
  const statement = outdent(`
    root {
      item
    }
...
it('parses empty argument lists, empty list/object values, and unknown escapes', () => {
  const statement = outdent(`
    query {
      noArgs()
      payload(
