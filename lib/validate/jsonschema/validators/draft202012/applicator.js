...
export default {
  prefixItems: (value, instance, validate) => {
    ...
    for (let i = 0; i < length; i++) {
      const subErrors = validate(value[i], instance[i])
      if (!subErrors.length) evaluateItem(instance[i])
  ...
  not: (value, instance, validate) => {
    const evaluatedPropertiesBackup = new Set(
      ...sessionDatabase.get('evaluatedProperties').slice(-1))
    const evaluatedItemsBackup = new Set(
    ...
    sessionDatabase.get('evaluatedItems').push(evaluatedItemsBackup)
    sessionDatabase.get('evaluatedProperties').pop()
