...
it('validate: uses function schemas by default', () => {
  ...
  assert.throws(() => validate(schema, instance, { eager: true }), { message: 'NaN' })
...
it('validate: defaults to non-eager validation', () => {
  const schema = {
    '*name': String,
    '*age': parseInt
  }
  const instance = [{ age: 'young' }]
...
it('validate: forwards eager option to default validator', () => {
  const schema = {
    '*name': String,
    '*age': parseInt
  }
  ...
  assert.throws(
    () => validate(schema, instance, { eager: true }),
