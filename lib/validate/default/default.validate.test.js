...
it('aggregates invalid record collection input when abortEarly is false', () => {
    const schema = {
      quantity: parseInt,
      price: parseFloat
    }
    ...
    let error = null
    try {
      validateDefault(schema, null, { abortEarly: false })
    } catch (exception) {
      error = exception
    }
...
it('strict mode validates additional properties on nested objects', () => {
    ...
    assert.throws(
      () => validateDefault(schema, records, { strict: true }),
...
it('collects all validation issues when abortEarly is false', () => {
    const schema = {
      '*name': String,
      '*age': parseInt,
      levels: [parseInt],
    ...
    let error = null
    try {
      validateDefault(schema, records, { strict: true, abortEarly: false })
    } catch (exception) {
      error = exception
    }
...
it('collects validator exceptions when abortEarly is false', () => {
    const schema = {
      levels: [value => {
        if (value < 0) {
    ...
    let error = null
    try {
      validateDefault(schema, records, { abortEarly: false })
    } catch (exception) {
      error = exception
    }
